...
private struct FlatFileRowView: View {
  ...
  private var isSelectedTreeItem: Bool {
    node.path == model.selectedFileTreePath
  }
...
enum FileDropHelper {
    ...
    for provider in providers {
      ...
      if provider.hasItemConformingToTypeIdentifier("public.file-url") {
        provider.loadItem(forTypeIdentifier: "public.file-url", options: nil) { data, _ in
          ...
          DispatchQueue.main.async {
            processFile(
              source: sourcePath, targetDir: targetDir,
  ...
  private static func processFile(
    ...
    let destPath = ((targetDir as NSString).appendingPathComponent(sourceName) as NSString)
      .standardizingPath

    // Can't drop onto itself or into itself.
    guard normalizedSource != destPath,
      !destPath.hasPrefix(normalizedSource + "/")
    else { return }

...
extension NSToolbarItem.Identifier {
  ...
  fileprivate static let automation = NSToolbarItem.Identifier("automation")
  fileprivate static let keybindings = NSToolbarItem.Identifier("keybindings")
  fileprivate static let languageServers = NSToolbarItem.Identifier("languageServers")
}
...
final class SettingsWindowController: NSWindowController {
  ...
  private func makeEditorPane() -> NSView {
    ...
    let ligaturesCheck = NSButton(
      checkboxWithTitle: "Font ligatures",
      target: self, action: #selector(fontLigaturesChanged(_:)))
    ligaturesCheck.state = settings.fontLigatures ? .on : .off

    addSection(
      to: stack, title: "Font", subtitle: "Editor typeface and size",
      rows: [
    ...
    let rightMarginCheck = NSButton(
      checkboxWithTitle: "Show right margin",
    ...
    let lineHeightField = NSTextField(
      string: settings.editorLineHeight > 0 ? "\(settings.editorLineHeight)" : "Default")
    ...
    addSection(
      to: stack, title: "Display", subtitle: "Visual appearance of the editor",
      rows: [
        lineNumbersCheck,
        wordWrapCheck,
        minimapCheck,
    ...
    let cursorPopup = NSPopUpButton(
      title: "", target: self, action: #selector(cursorStyleChanged(_:)))
    cursorPopup.addItems(withTitles: [
      "line", "block", "underline", "line-thin", "block-outline", "underline-thin",
    ])
    cursorPopup.selectItem(withTitle: settings.editorCursorStyle)

    let blinkPopup = NSPopUpButton(
      title: "", target: self, action: #selector(cursorBlinkingChanged(_:)))
    ...
    let autoClosePopup = NSPopUpButton(
      title: "", target: self, action: #selector(autoClosingBracketsChanged(_:)))
    ...
    let foldingCheck = NSButton(
      checkboxWithTitle: "Code folding",
      target: self, action: #selector(foldingChanged(_:)))
    foldingCheck.state = settings.folding ? .on : .off

    let autoSaveCheck = NSButton(
      checkboxWithTitle: "Auto save on focus loss",
  ...
  private func makeTerminalPane() -> NSView {
    ...
    addSection(
      to: stack, title: "Font", subtitle: "Terminal typeface and size",
      rows: [
        makeRow(label: "Font Family:", control: fontFamilyField),
        makeRow(label: "Font Size:", control: fontSizeRow),
      ], addSeparator: false)
    ...
    let cursorBlinkCheck = NSButton(
      checkboxWithTitle: "Cursor blink",
    ...
    let copyOnSelectCheck = NSButton(
      checkboxWithTitle: "Copy on select",
    ...
    let longCommandCheck = NSButton(
      checkboxWithTitle: "Request attention after long commands",
    ...
    addSection(
      to: stack, title: "Scrollback", subtitle: "Number of lines kept in the scroll buffer",
      rows: [
  ...
  private func makeAppearancePane() -> NSView {
    ...
    let schemePopup = NSPopUpButton(
      title: "", target: self, action: #selector(colorSchemeChanged(_:)))
    ...
    addSection(
      to: stack, title: "Color Scheme", subtitle: "Theme applied to editor, terminal, and UI",
      rows: [
  ...
  private func makeAutomationPane() -> NSView {
    ...
    let removeCmdButton = NSButton(
      title: "Remove", target: self, action: #selector(removeCommandOnSave(_:)))
    ...
    addSection(
      to: stack, title: "Commands on Save",
    ...
    let addFtoButton = NSButton(
      title: "Add", target: self, action: #selector(addFileTypeOverride(_:)))
    let removeFtoButton = NSButton(
      title: "Remove", target: self, action: #selector(removeFileTypeOverride(_:)))
    let ftoButtonRow = NSStackView(views: [addFtoButton, removeFtoButton])
    ftoButtonRow.orientation = .horizontal
    ftoButtonRow.spacing = 8

    addSection(
      to: stack, title: "File Type Overrides",
  ...
  private func makeKeybindingsPane() -> NSView {
    ...
    let resetButton = NSButton(
      title: "Reset All to Defaults", target: self,
      action: #selector(resetKeybindings(_:)))

    addSection(
      to: stack, title: "Built-in Keybindings",
    ...
    let addCustomButton = NSButton(
      title: "Add", target: self, action: #selector(addCustomKeybinding(_:)))
    let removeCustomButton = NSButton(
      title: "Remove", target: self, action: #selector(removeCustomKeybinding(_:)))
    ...
    addSection(
      to: stack, title: "Custom Keybindings",
      subtitle:
        "User-defined shortcuts that open a new terminal and run a command. Double-click to edit.",
  ...
  private func makeLanguageServersPane() -> NSView {
    ...
    let npmLabel = NSTextField(
      labelWithString:
        npmAvailable
        ? "npm is available" : "npm not found — install Node.js to manage web language servers")
    ...
    addSection(
      to: stack, title: "Managed Web Language Servers",
    ...
    addSection(
      to: stack, title: "System Language Servers",
  ...
  @objc private func installWebLspServers(_ sender: NSButton) {
    ...
    func findSpinner(in view: NSView) -> NSProgressIndicator? {
      if let pi = view as? NSProgressIndicator,
        pi.identifier == NSUserInterfaceItemIdentifier("lspInstallSpinner")
      {
        return pi
      }
  ...
  private func addSection(
    to stack: NSStackView, title: String, subtitle: String? = nil,
  ...
  @objc private func colorSchemeChanged(_ sender: NSPopUpButton) {
    ...
    if let previewBox = findView(withIdentifier: "colorPreviewBox", in: window?.contentView)
      as? NSBox
    {
      previewBox.fillColor = theme.bgColor
      previewBox.contentView = buildColorPreview(theme: theme)
  ...
  @objc private func addCommandOnSave(_ sender: Any?) {
    settings.commandsOnSave.append(
      CommandOnSave(
        name: "New Command",
        command: "",
        args: [],
  ...
  @objc private func fileTypeOverrideDoubleClicked(_ sender: NSTableView) {
    ...
    SettingsFormSheet.present(
      ...
      let fmtArgs = (values["fmtArgs"] ?? "")
        .split(separator: " ")
        .map(String.init)
      let formatOnSave: FormatOnSave? =
        fmtCmd.isEmpty ? nil : FormatOnSave(command: fmtCmd, args: fmtArgs)
  ...
  @objc private func addCustomKeybinding(_ sender: Any?) {
    settings.customKeybindings.append(
      CustomKeybinding(
        name: "New Keybinding",
        key: "",
        command: "",
...
extension SettingsWindowController: NSTableViewDataSource, NSTableViewDelegate {
  ...
  func tableView(_ tableView: NSTableView, viewFor tableColumn: NSTableColumn?, row: Int) -> NSView?
  {
    ...
    case 600:
      ...
      case "fto_use_spaces":
        if let s = fto.useSpaces {
          cell.stringValue = s ? "Yes" : "No"
        } else {
          cell.stringValue = "-"
        }

...
- [x] Define `CommandPaletteItem`, stable item IDs, and `RecentCommandItem` in `impulse-core`.
...
**Implementation note:** The first shared protocol pass adds stable path-based node IDs, `FileTreePatch` remove/upsert operations, child-diff generation, watcher-event batching, and view-state reconciliation helpers in `impulse-core`. Frontend patch application remains next.
